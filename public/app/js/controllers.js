...
var odfeControllers = angular.module('odfeControllers', ['d3']);

//want a service to get the gauges data
odfeControllers.service('ODFEGaugesService', ['$scope', '$routeParams', '$http',
    function($scope, $routeParams, $http) {
		$scope.doc = $routeParams.odfName;
        $http.get('records/' + $routeParams.odfName + '/odferuns.json').
        success(function(data) {
        	$scope.runs = data.runs;

        }).
        error(function(data, status, headers, config) {
        	alert("http error " + status + " reading " + 'records/' + $routeParams.odfName + '/odferuns.json')
        });

	}]);


odfeControllers.directive('gaugeChart', ['d3Service', function(d3Service) {
	    return {
	        restrict: 'E',
	        link: function(scope, element, attrs) {
	          d3Service.d3().then(function(d3) {
					d3.select("#chart").append("h1").text("Add data here");
...
odfeControllers.controller('ODFEPathsCtrl', ['$scope', '$routeParams', '$http',  'd3Service',
          function($scope, $routeParams, $http,  d3Service) {
			...
			d3Service.d3().then(function(d3) {
				...
				var jsonfile = "records/" + $scope.doc + "/" + $scope.extract + "/xpath.json";
...
odfeControllers.service('$fileUpload', ['$http', function($http) {
